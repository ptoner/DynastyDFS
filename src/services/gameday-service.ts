...
class GamedayService {
    ...
    async loadStoreForWallet(walletAddress:string) {
        this.scoreboardDb = await this.schemaService.getScoreboardStoreByWalletAddress(walletAddress)
        this.boxscoreDb =  await this.schemaService.getBoxscoreStoreByWalletAddress(walletAddress)
    }
    ...
    async updatePlayers(playerIds:any[], date: Date) : Promise<void> {
        ...
        for (let playerId of playerIds) {
            ...
            let existing: Player = await this.playerService.get(playerId)
            ...
            if (!existing) {
                ...
                await this.playerService.put(player)
    ...
    async downloadSchedule(date: Date) : Promise<void> {
        ...
        try {
            ...
            gamedayScoreboard.date = formattedDate

            if (gamedayScoreboard.id) {
                await this.scoreboardDb.put(formattedDate, gamedayScoreboard)
            } else {
    ...
    async load() {
      await this.boxscoreDb.load()
      await this.scoreboardDb.load()
    }
